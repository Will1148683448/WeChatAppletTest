<!-- 对uview2的u-upload组件进行了修改： -->
<!-- 1.增加了选中图片的圆角属性borderRadius -->
<!-- 2.加入了删除按钮的自定义图片和自定义样式属性 -->
<template>
	<view class="u-upload" :style="[$u.addStyle(customStyle)]">
		<view class="u-upload__wrap">
			<template v-if="previewImage">
				<view class="u-upload__wrap__preview" v-for="(item, index) in lists" :key="index" :style="{
					...
					<view v-else class="u-upload__wrap__preview__other">
						...
						<text class="u-upload__wrap__preview__other__text">{{ item.isVideo || (item.type && item.type ===
							'video') ? '视频' : '文件'}}</text>
					</view>
					...
					<view class="u-upload__deletable"
						...
						<!-- 默认删除按钮 -->
						<view v-if="deleteBtnSrc==''" style="padding: 6rpx; width: 34rpx; height: 34rpx; background: rgba(255,0,0,0.7); border-radius: 50%;">
							<u-icon  name="close" color="#fff" size="22"></u-icon>
						</view>
						<!-- 自定义删除按钮 -->
						<image v-else :style="{
								height: deleteBtnStyle.height,
								width: deleteBtnStyle.width,
								position: 'absolute',
								top: deleteBtnStyle.top,
								right: deleteBtnStyle.right
							}" :src="deleteBtnSrc" mode="scaleToFill"/>
					</view>
...
import {
	chooseFile
} from '@/uni_modules/uview-ui/components/u-upload/utils';
import mixin from '@/uni_modules/uview-ui/components/u-upload/mixin.js';
